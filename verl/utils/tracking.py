...
class Tracking(object):
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = 'console', config=None):
        ...
        if 'tensorboard' in default_backend:
            ...
            import os
            log_dir = os.path.join(experiment_name, "tensorboard")
            os.makedirs(log_dir, exist_ok=True)
            self.tensorboard_writer = SummaryWriter(log_dir=log_dir)
            self.logger['tensorboard'] = _TensorBoardLoggingAdapter(self.tensorboard_writer)
...
class _TensorBoardLoggingAdapter:
    ...
    def log(self, data, step):
        for key, value in data.items():
            ...
            elif hasattr(value, 'shape') and len(value.shape) == 0:  # scalar tensor

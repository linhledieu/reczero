...
def _safe_normalize_batch_size(value: int, divisor: int, field_name: str) -> int:
    """Normalize a global batch config to per-rank and avoid invalid zero values."""
    if divisor <= 0:
        raise ValueError(f"Invalid divisor={divisor} when normalizing {field_name}")
    normalized = value // divisor
    if normalized <= 0:
        logger.warning(
            "%s=%s is smaller than data-parallel divisor=%s; clamping normalized value to 1. "
            "Set %s >= %s to avoid this fallback.",
            field_name,
            value,
            divisor,
            field_name,
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self._is_actor:
            self.config.actor.ppo_mini_batch_size = _safe_normalize_batch_size(
                value=self.config.actor.ppo_mini_batch_size,
                divisor=dp_divisor,
                field_name='actor.ppo_mini_batch_size',
            )
            self.config.actor.ppo_micro_batch_size = _safe_normalize_batch_size(
                value=self.config.actor.ppo_micro_batch_size,
                divisor=dp_divisor,
                field_name='actor.ppo_micro_batch_size',
            )
            self.config.actor.ppo_mini_batch_size *= self.config.rollout.n
            self.config.actor.ppo_micro_batch_size *= self.config.rollout.n
        if self._is_rollout:
            self.config.rollout.log_prob_micro_batch_size = _safe_normalize_batch_size(
                value=self.config.rollout.log_prob_micro_batch_size,
                divisor=dp_divisor,
                field_name='rollout.log_prob_micro_batch_size',
            )
            self.config.rollout.log_prob_micro_batch_size *= self.config.rollout.n
        if self._is_ref:
            self.config.ref.log_prob_micro_batch_size = _safe_normalize_batch_size(
                value=self.config.ref.log_prob_micro_batch_size,
                divisor=dp_divisor,
                field_name='ref.log_prob_micro_batch_size',
            )
...
class CriticWorker(Worker):
    ...
    def __init__(self, config):
        ...
        # normalize config
        dp_divisor = torch.distributed.get_world_size() // self.ulysses_sequence_parallel_size
        self.config.ppo_mini_batch_size = _safe_normalize_batch_size(
            value=self.config.ppo_mini_batch_size,
            divisor=dp_divisor,
            field_name='critic.ppo_mini_batch_size',
        )
        self.config.ppo_micro_batch_size = _safe_normalize_batch_size(
            value=self.config.ppo_micro_batch_size,
            divisor=dp_divisor,
            field_name='critic.ppo_micro_batch_size',
        )
        self.config.forward_micro_batch_size = _safe_normalize_batch_size(
            value=self.config.forward_micro_batch_size,
            divisor=dp_divisor,
            field_name='critic.forward_micro_batch_size',
        )

...
CKPT_ROOT="/data/uqlinh/reczero/checkpoints/music"
...
TEST_FREQ=100
TOTAL_EPOCHS=1
EARLY_STOP_MAE=${EARLY_STOP_MAE:-}   # e.g. export EARLY_STOP_MAE=0.85 before running
...
CHECK_GPUS=(1 3)          # Physical GPU IDs to wait for before launch
CHECK_INTERVAL=300        # 5 minutes
MIN_FREE_MEM_MIB=40960    # 40 GB free on each target GPU
...
while (( attempt < MAX_RETRIES )); do
  ...
  python3 -m verl.trainer.main_ppo \
    ...
    data.train_files=/data/uqlinh/reczero/data/music/train.parquet \
    data.val_files=/data/uqlinh/reczero/data/music/test.parquet \

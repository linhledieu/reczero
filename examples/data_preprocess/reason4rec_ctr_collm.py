...
# 读取原始数据
data_df = pd.read_pickle(f'/home/huangyanwen.hyw/code_linlin/CoLLM-main/book/test_ood2.pkl')
...
data_df["user_history"] = None       # dtype=object，可存 list
data_df["item_history"] = None
data_df["his_len"]      = 0
...
for idx, row in tqdm(data_df.iterrows(), total=len(data_df)):
    ...
    item_id = row['iid']
    user_history = row['his_title']
    # user_history = history_df[history_df['uid'] == user_id]
    ...
    # 存储需要的字段，包含统计信息
    data_df.at[idx, 'user_history'] = user_history
    data_df.at[idx, 'item_history'] = item_history
    data_df.at[idx, 'ground_truth'] = row['flag']
    data_df.at[idx, 'label'] = row['label']
...
# 保存为 JSONL 文件
with open(f'/home/huangyanwen.hyw/code_linlin/Logic-RL-rating/data/Reason4Rec/{dataset}/raw_features_data_ctr_collm_test.jsonl', 'w', encoding='utf-8') as f:
    for entry in jsonl_data:
        f.write(json.dumps(entry, ensure_ascii=False) + '\n')

print(f"Generated {len(data_df_sample)} samples using raw features")
print(f"Saved JSONL to: /home/huangyanwen.hyw/code_linlin/Logic-RL-rating/data/Reason4Rec/{dataset}/raw_features_data_ctr_collm_test.jsonl")

...
# 读取原始数据
data_df = pd.read_pickle(f'/home/huangyanwen.hyw/code_linlin/Logic-RL-rating/data/Reason4Rec/{dataset}/train_summarizer_generation_results.pkl')
...
for idx, row in tqdm(data_df.iterrows(), total=len(data_df)):
    ...
    for i, (_, his) in enumerate(user_history.iterrows()):
        ...
        aspect_preference_summary = his['aspect_preference_summary']
        user_history_text += f"{aspect_preference_summary}\n\n"
    user_history_text = user_history_text.strip()
    
    # 构建商品历史文本 - 使用聚合格式
    item_history_text = format_aggregated_item_history(item_history)
    # item_history_text = item_history

    # 存储需要的字段，包含统计信息
    data_df.at[idx, 'user_history'] = user_history_text
    data_df.at[idx, 'item_history'] = item_history_text
    data_df.at[idx, 'ground_truth'] = target_rating
    data_df.at[idx, 'user_avg'] = user_average_rating
    data_df.at[idx, 'item_avg'] = item_average_rating
...
# 保存为 JSONL 文件
with open(f'/home/huangyanwen.hyw/code_linlin/Logic-RL-rating/data/Reason4Rec/{dataset}/raw_features_data_teacher.jsonl', 'w', encoding='utf-8') as f:
    for entry in jsonl_data:
        f.write(json.dumps(entry, ensure_ascii=False) + '\n')

print(f"Generated {len(data_df_sample)} samples using raw features")
print(f"Saved JSONL to: /home/huangyanwen.hyw/code_linlin/Logic-RL-rating/data/Reason4Rec/{dataset}/raw_features_data_teacher.jsonl")
...
if len(jsonl_data) > 0:
    ...
    print(f"User Average Rating: {example['user_avg']:.2f}")
    print(f"Item Average Rating: {example['item_avg']:.2f}")
